...
if __name__ == "__main__":
    ...
    # 💡 기본값 경로를 Naver에서 Kakao로 변경합니다.
    parser.add_argument('--input_dir', type=str, default=r'D:\Crawling\Kakao', 
                         help='원본 이미지 디렉토리 경로 (기본값: D:\Crawling\Kakao)')
    parser.add_argument('--output_dir', type=str, default=r'D:\Crawling\Kakao_Processed',
                         help='전처리 결과 저장 경로 (기본값: D:\Crawling\Kakao_Processed)')
    parser.add_argument('--target_size', type=int, default=320,
                         help='리사이즈 목표 크기 (기본값: 320)')
    ...
    parser.add_argument('--create_zip', action='store_true',
                         help='결과를 ZIP으로 압축 (현재 미구현 기능)') # 주석 처리된 기능은 제외
    parser.add_argument('--visualize', action='store_true',
                         help='전처리 전후 비교 이미지 생성 (현재 미구현 기능)') # 주석 처리된 기능은 제외
    
    args = parser.parse_args()
    
    # 전처리 실행
    print(f"\n{'='*60}")
    print(f"🎨 웹툰 컷 이미지 전처리 시작 (Kakao 경로)")
    print(f"{'='*60}")
    print(f"📂 입력 경로: {args.input_dir}")
    print(f"📂 출력 경로: {args.output_dir}")
    print(f"🎯 목표 크기: {args.target_size}x{args.target_size}px")
